...
import { Camera, EndedRenderEvent, Entity, EntityContainer, FullRerenderEvent, RerenderEvent, SystemContainerInterface, Vector, Viewbox } from '../../lib'
...
export default class EcsEditor extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    let toRender = <div id={"ecs-editor-root"} style={{
      display: "flex",
      overflow: 'hidden'
    }}
    ...
    >
      {this.props.systemContainer.getActive().map((system, key) => {
        console.log("reactRender for "+system.getName());
